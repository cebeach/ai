...
| Revision | r7 |
| Fingerprint | 59aa87620c605c0b259450ebc4fce2e8da36f316d4064da240dc59d84f0e319a |
| Status | draft |
| Timestamp | 2026-03-16T03:38:31 |
...
```
total_tokens =
    tokens(system_prompt_and_instructions)
  + tokens(history)
  + tokens(response)
```

Note: in most APIs the system prompt is the instruction layer; they are
counted together as a single term, not separately.

When total_tokens approaches the context window limit, the session must be
checkpointed and restarted. Truncation behavior when the limit is exceeded
is implementation-defined and varies across APIs: some return an error,
some truncate from the beginning of the conversation history, and some
truncate from the middle. Do not rely on any specific truncation strategy.
